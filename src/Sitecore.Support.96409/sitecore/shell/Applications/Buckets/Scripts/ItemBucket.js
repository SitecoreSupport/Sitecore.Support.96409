    ...
    $j.each(resultCallBack.items,
        function () {
            ...
            if (this.Name != null) {
                ...
                //Sitecore.Support.96409
                var template = '<div id="' + this.ItemId + '" class="BlogPostArea" onclick="{0}" style="margin-left:' + InnerItem(this) + '"><div class="BlogPostViews">' + '<a class="ceebox imgcontainer"  title="" href="#"  onclick="{0}"><img onerror="this.onerror=null;this.src=\'../Buckets/images/default.jpg\';" src="' + this.ImagePath + '?w=' + imageWidth + '&h=' + imageHeight + '&db=master" ' + resizeTemplateIcon + '  class="attachment-post-thumbnail wp-post-image" alt="' + this.Name + '" title="' + this.Name + ' - ' + this.Path + '" /></a></div><h5 class="BlogPostHeader"><a href="javascript:void(0);" onclick="scForm.browser.clearEvent(event || window.event, true);{0}">' + (this.DisplayName || this.Name) + '</a><span title="This item has ' + (this.Languages && this.Languages.length > 1 ? "" + this.Languages.length + " languages" : " 1 language") + '">' + (this.Languages && this.Languages.length > 1 ? "(" + this.Languages.length + ")" : "") + '</span></h5><div class="BlogPostContent"><strong>' + templateStub + ': </strong>' + this.TemplateName + ' <strong>' + locationStub + ': </strong>' + this.Bucket + '</div><div class="BlogPostFooter">' + (this.Content ? this.Content : '') + ' <div><strong>' + versionStub + ': </strong>' + this.Version + ' <strong>' + createdStub + ': </strong>' + this.CreatedDate.substring(0, 10) + ' <strong> ' + byStub + ': </strong> ' + this.CreatedBy + ' <strong> ' + languageStub + ': </strong> ' + this.Language + ' </div><div></div><div class="fetchChildren" onclick="scForm.browser.clearEvent(event || window.event, true);fetchChildren(this);" style="float: right;padding: 4px 6px;" title="Fetch Child Items">' + hasChildren + '</div></li>';
                //
                switch (window.currentBucketsViewType) {
                    case window.rteViewType:
